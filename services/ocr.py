...
class OcrService:
    def read(self, upload: BinaryIO) -> OcrResult:
        ...
        variants = [image, ImageOps.autocontrast(image).resize((image.width * 2, image.height * 2))]
        for attempt, candidate in enumerate(variants[:settings.OCR_MAX_ATTEMPTS], start=1):
            try:
                data = pytesseract.image_to_data(candidate, config="--psm 6", output_type=pytesseract.Output.DICT)
            except Exception as error:
        ...
        return OcrResult(best_text, round(best_confidence, 1), min(len(variants), settings.OCR_MAX_ATTEMPTS), round((time.perf_counter() - started) * 1000, 1))
